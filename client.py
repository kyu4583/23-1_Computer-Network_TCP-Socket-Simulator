...
def threeway_first():
    global source_port, dest_port, timer, seq_num, receive_buffer_size, timeout_sec
    packet=Packet(source_port, dest_port, seq_num, 0, 0, 1, 0, receive_buffer_size, 0, "%%%")
    send_packet(packet)
    timer=time.time()+timeout_sec
    print("3way hanshake: SYN 전송 완료")


def move_window(need_for_ACK):
    global last_ACK, send_buffer, send_buffer_size, storage, timeout_sec
...
def make_data():
    ...
    with open("client_input.txt", 'r+') as f:
        contents = f.read()
        if contents == "":
            return 0
        f.seek(0)
...
while True:
    ...
    while True:
        with open(source_file, "r") as f:
            ...
            if temp:
                receive_buffer+=list(temp[0].data)
                if temp[0].checksum_error==1:
                    temp=temp[1:]
                    continue
                if temp[0].SYN_bit==1 and temp[0].ACK_bit==1:
                    #last_ACK = seq_num = temp[0].ACK_num
                    last_ACK = temp[0].ACK_num
                    received_seq = temp[0].seq_num
                    print("3way hanshake: SYNACK 수신 완료")
                    print("seq_num of server: {}\n".format(received_seq))
                    #packet=Packet(source_port, dest_port, seq_num, received_seq+data_len, 1, 0, 0, receive_buffer_size, 0, "%%%")
                    #send_buffer.append([packet,time.time()+timeout_sec])
                    #send_packet(packet)
                    break
                else:
                    continue
